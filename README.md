...
Each of these can be turned off at runtime. Moderators are exempt from spam protection and from `/raid kick_recent` — but **deliberately not from anti-nuke**, since a compromised moderator account is exactly what it exists to catch.

- **Version announcements.** On the first start after the running commit changes, the moderators channel gets a short "updated `v1.0.0` → `v1.1.0`" notice with the release notes. This is what confirms an update actually *landed* — the existing notices are sent before the restart, so a bot that failed to come back up looked identical to one that succeeded. It also covers manual `git pull` updates, which previously announced nothing. A rollback is labelled as such rather than shown as an upgrade. The body is the annotated tag's message, so release notes are whatever you write when tagging. Toggle with `VERSION_ANNOUNCE_ENABLED`. See the **[Safety Systems](../../wiki/Safety-Systems)** wiki page for thresholds, tuning, and the required bot permissions (**Manage Server**, **Moderate Members**, **Kick Members**, **Manage Roles**).
